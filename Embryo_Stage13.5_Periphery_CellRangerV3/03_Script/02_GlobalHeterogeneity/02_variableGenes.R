...
sc10x.rna.seurat = FindVariableFeatures( object = sc10x.rna.seurat, 
...
variablesGenesStats = paste(length( VariableFeatures( sc10x.rna.seurat)), "/", nrow( sc10x.rna.seurat));
...
suppressMessages(suppressWarnings(LabelPoints( plot = VariableFeaturePlot( sc10x.rna.seurat) + theme(legend.position = "none"), 
                                               points = head( VariableFeatures( sc10x.rna.seurat), 10), 
                                               repel = TRUE)));
...
variableAnnotationsDT = head( HVFInfo( object = sc10x.rna.seurat, assay = "RNA", selection.method = 'vst')[VariableFeatures( sc10x.rna.seurat),], VARIABLE_FEATURES_SHOWTOP);
